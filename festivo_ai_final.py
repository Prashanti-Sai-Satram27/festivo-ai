...
def send_feedback(message_id, rating, content=None):
    feedback_url = DIFY_FEEDBACK_URL_TEMPLATE.format(message_id=message_id)
    headers = {
    ...
    requests.post(feedback_url, headers=headers, json=payload)
...
st.markdown(f"""
    <style>
        ...
        /* ✨ Fix for Centering Entire App Content */
